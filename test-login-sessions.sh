...
# Comprehensive Login Session Test Script
echo "🧪 Testing Login Sessions with Token Refresh"
echo "============================================="

# Configuration
API_BASE_URL="https://dtcvfgct9ga1.cloudfront.net"
TEST_EMAIL="session-test@example.com"
...
logout_user() {
    ...
    ACCESS_TOKEN=""
    REFRESH_TOKEN=""
    echo "✅ Logged out (tokens cleared)"
}

# Function to test token refresh
test_token_refresh() {
    echo "🔄 Testing token refresh..."
    local refresh_response=$(make_request "POST" "/auth/refresh" "{\"refreshToken\":\"$REFRESH_TOKEN\"}")
    
    if echo "$refresh_response" | grep -q "accessToken"; then
        local new_access_token=$(extract_json_value "$refresh_response" "accessToken")
        local new_refresh_token=$(extract_json_value "$refresh_response" "refreshToken")
...
echo "1️⃣ Registering test user (if not exists)..."
...
while true; do
    echo ""
    echo "🔄 DELETION SESSION - Removing boards"
    echo "-------------------------------------"
    
    # Login
    if ! login_user; then
        echo "❌ Deletion session failed at login"
        break
    fi
    ...
    echo "✅ Deletion session completed"
    sleep 2  # Brief pause between sessions
done
...
echo "  ✅ Token refresh tested in each session"
echo "  ✅ Proper login/logout flow maintained"
echo ""
echo "🎉 All tests completed successfully!"
echo ""
echo "💡 Key Observations:"
echo "  - Token refresh mechanism working correctly"
